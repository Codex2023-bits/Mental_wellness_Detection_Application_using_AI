<%-- Update the top of profile.jsp --%>
    <%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
        <% String email=(String) session.getAttribute("user"); String loginTime=(String)
            ...
            <body>
                    ...
                    <div class="prof-content">
                        ...
                        <div class="prof-grid">
                            ...
                            <div class="prof-card">
                                ...
                                <div class="prof-info-row">
                                    <span class="prof-info-label"><i class="fa-solid fa-clock"></i> Session
                                        Started</span>
                                    <span class="prof-info-value">
                                ...
                                <div class="prof-info-row">
                                    <span class="prof-info-label"><i class="fa-solid fa-fingerprint"></i> Session
                                        ID</span>
                ...
                <script>
                    ...
                    document.getElementById('refreshWellnessBtn').addEventListener('click', function () {
                        ...
                        document.getElementById('popupStatus').style.color = '#374151'; // Reset color
                        ...
                        // Fetch AI report for score, status, suggestions and quote
                        fetch('report-api')
                            .then(r => r.json())
                            .then(data => {
                                loading.style.display = 'none';
                                if (data.report) {
                                    // Extract custom SCORE and STATUS from the report string
                                    let aiScore = 0;
                                    let aiStatus = "Analyzing...";

                                    const scoreMatch = data.report.match(/\[SCORE:\s*(\d+)(?:\/10)?\]/i);
                                    if (scoreMatch && scoreMatch[1]) {
                                        aiScore = parseInt(scoreMatch[1], 10);
                                    } else {
                                        // Fallback calculation if AI missed the score tag
                                        aiScore = 5;
                                    }

                                    const statusMatch = data.report.match(/\[STATUS:\s*([^\]]+)\]/i);
                                    if (statusMatch && statusMatch[1]) {
                                        aiStatus = statusMatch[1].trim();
                                    } else {
                                        aiStatus = aiScore >= 8 ? 'Thriving 🌟' :
                                            aiScore >= 6 ? 'Healthy 💚' :
                                                aiScore >= 3 ? 'Developing 🌱' : 'Getting Started 🌸';
                                    }

                                    currentWellnessScore = aiScore;

                                    // Animate score ring based on AI score
                                    ...
                                    if (overallRow) {
                                        ...
                                        if (cells.length >= 2) {
                                            ...
                                            cells[2].innerHTML = '<span style="color:' + color + '; font-weight:500;">' + aiStatus + '</span>';
                                        }
                                    }

                                    // Remove the specific SCORE and STATUS lines from the shown AI report output
                                    let cleanedReport = data.report
                                        .replace(/\[SCORE:[^\]]*\]/ig, '')
                                        .replace(/\[STATUS:[^\]]*\]/ig, '')
                                        .trim();
                                    ...
                                    if (suggestionItems.length === 0) {
                                        const sentences = cleanedReport.split('.').filter(s => s.trim().length > 15);
                                    ...
                                    if (overallRow) {
                                        const cells = overallRow.querySelectorAll('td');
                                        if (cells.length >= 4) {
                                            const remark = cleanedReport.split('.')[0] + '.';
